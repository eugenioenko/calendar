...
let DayViewComponent = class DayViewComponent {
    ...
    addReminder() {
        const modalRef = this.modalService.open(_modal_reminders_modal_reminders_component__WEBPACK_IMPORTED_MODULE_3__["ModalRemindersComponent"]);
...
let ModalRemindersComponent = class ModalRemindersComponent {
    constructor(activeModal, formBuilder, calendarService, weatherService) {
    ...
    createEditForm() {
        ...
        const momentDate = moment__WEBPACK_IMPORTED_MODULE_5__(new Date(reminder.date));
        const time = {
...
let MonthSelectorComponent = class MonthSelectorComponent {
    ...
    newReminder() {
        const modalRef = this.modalService.open(_modal_reminders_modal_reminders_component__WEBPACK_IMPORTED_MODULE_4__["ModalRemindersComponent"]);
    }
};
...
let MonthViewComponent = class MonthViewComponent {
    ...
    updateCalendar() {
        this.calendar = [];
        const date = moment__WEBPACK_IMPORTED_MODULE_2__([this.year, this.month]);
        const startWeek = date.startOf('month').week();
        const endWeek = startWeek + this.weeksToDisplay;
        for (let week = startWeek; week < endWeek; ++week) {
            this.calendar.push(Array(7).fill(0).map((n, dayOfWeek) => moment__WEBPACK_IMPORTED_MODULE_2__([this.year])
                .week(week)
                .startOf('week')
                .add(dayOfWeek, 'day')));
        }
...
const calendarReducer = (state = initial, { type, payload }) => {
    switch (type) {
        case 'date_change':
